...
namespace CourseFeedbackSystem.Web.Host.Startup
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            services.AddAbpWithoutCreatingServiceProvider<CourseFeedbackSystemWebHostModule>(
...
app.Use(async (context, next) =>                {                    await next();                    if (context.Response.StatusCode == 404                        && !Path.HasExtension(context.Request.Path.Value)                        && !context.Request.Path.Value.StartsWith("/api/services", StringComparison.InvariantCultureIgnoreCase))                    {                        context.Request.Path = "/index.html";                        await next();                    }                });
            ...
            app.UseSwaggerUI(options =>
            {
                ...
                options.SwaggerEndpoint($"/swagger/{_apiVersion}/swagger.json", $"CourseFeedbackSystem API {_apiVersion}");
                options.IndexStream = () => Assembly.GetExecutingAssembly()
                    .GetManifestResourceStream("CourseFeedbackSystem.Web.Host.wwwroot.swagger.ui.index.html");
        ...
        private void ConfigureSwagger(IServiceCollection services)
        {
            services.AddSwaggerGen(options =>
            {
                ...
                {
                    Version = _apiVersion,
                    Title = "CourseFeedbackSystem API",
                    Description = "CourseFeedbackSystem",
                    // uncomment if needed TermsOfService = new Uri("https://example.com/terms"),
                    Contact = new OpenApiContact
                    {
                        Name = "CourseFeedbackSystem",
                        Email = string.Empty,
                ...
                if (canShowSummaries)
                {
                    ...
                    var applicationXml = $"CourseFeedbackSystem.Application.xml";
                    var applicationXmlPath = Path.Combine(AppContext.BaseDirectory, applicationXml);
                    options.IncludeXmlComments(applicationXmlPath);

                    var webCoreXmlFile = $"CourseFeedbackSystem.Web.Core.xml";

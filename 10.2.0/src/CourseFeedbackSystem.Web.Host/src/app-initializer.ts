...
export class AppInitializer {
                        ...
                        appSessionService.init().then(
                            (result) => {
                                ...
                                if (this.shouldLoadLocale()) {
                                    ...
                                    // Try to import locale, but don't fail if it doesn't exist
                                    // The bundler may not support dynamic imports for all locales
                                    try {
                                        ...
                                        (module) => {
                                            ...
                                            resolve(result);
                                                ...
                                                (err) => {
                                                    // If locale import fails, continue without it (fallback to default)
                                                    console.warn(`Failed to load locale ${angularLocale}, using default:`, err);
                                                    resolve(result);
                                                }
                                            )
                                            .catch((err) => {
                                                // Additional catch for any unexpected errors
                                                console.warn(`Error loading locale ${angularLocale}:`, err);
                                                resolve(result);
                                            });
                                    } catch (err) {
                                        // If dynamic import is not supported, continue without locale
                                        console.warn(`Dynamic import not supported for locale ${angularLocale}:`, err);
                                        resolve(result);
                                    }
                                } else {
                                    resolve(result);
    ...
    private shouldLoadLocale(): boolean {
        const currentLang = abp.localization.currentLanguage?.name;
        // Don't load locale for English variants (en, en-US, etc.)
        return currentLang && !currentLang.toLowerCase().startsWith('en');
    }
